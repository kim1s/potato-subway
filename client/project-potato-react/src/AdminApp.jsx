...
export default function AdminApp() {
  ...
  useEffect(() => {
    fetch("/api/admin/config")
      .then((r) => r.json())
      .then((data) => {
        if (data.notionDatabaseUrl) setNotionUrl(data.notionDatabaseUrl);
      })
      .catch(() => {});
  }, []);
        ...
        <section className="bg-white border border-stone-200 p-6 mb-4">
          ...
          <div className="bg-stone-50 border border-stone-200 px-4 py-4 mb-6 text-xs text-stone-600 leading-relaxed">
            ...
            <ol className="list-decimal list-inside space-y-2">
              <li>
                ...
                <code className="font-mono bg-stone-200 px-1 text-stone-700">NOTION_TOKEN</code>을 서버 환경변수에 등록
              </li>
          ...
          <form onSubmit={handleImport} className="space-y-6">
            <div className="flex flex-col gap-2">
              <label
                htmlFor="notion-url"
                className="text-[0.5625rem] font-semibold uppercase tracking-widest text-stone-400"
              >
                Notion Database URL
              </label>
              <input
        ...
        {status === "done" && result && (
          <div className="border border-stone-200 bg-white px-4 py-4">
            ...
            <ul className="text-sm text-stone-700 space-y-1.5">
              <li>신규 등록: <strong className="font-semibold text-stone-900">{result.imported}건</strong></li>
              <li>업데이트: <strong className="font-semibold text-stone-900">{result.updated}건</strong></li>
            ...
            {result.message && (
              <p className="mt-3 text-xs text-stone-400">{result.message}</p>
            )}
          </div>
        )}
